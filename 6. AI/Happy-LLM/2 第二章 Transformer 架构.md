# [第二章 Transformer 架构](https://datawhalechina.github.io/happy-llm/#/./chapter2/第二章 Transformer架构?id=第二章-transformer-架构)

## [2.1 注意力机制](https://datawhalechina.github.io/happy-llm/#/./chapter2/第二章 Transformer架构?id=_21-注意力机制)

### [2.1.1 什么是注意力机制](https://datawhalechina.github.io/happy-llm/#/./chapter2/第二章 Transformer架构?id=_211-什么是注意力机制)
...
### [2.1.2 深入理解注意力机制](https://datawhalechina.github.io/happy-llm/#/./chapter2/第二章 Transformer架构?id=_212-深入理解注意力机制)
...
在第一章中，我们有提到词向量的概念。[通过合理的训练拟合，词向量能够表征语义信息，从而让语义相近的词在向量空间中距离更近，语义较远的词在向量空间中距离更远。](./知识储备/2 第二章 Transformer 架构/词向量.md)我们往往用欧式距离来衡量词向量的相似性，但我们同样也可以用点积来进行度量：
$$
...
attention(Q, K, V) = softmax(QK^T)v
$$
目前，我们离标准的注意力机制公式还差最后一步。在上一个公式中，如果 Q 和 K 对应的维度 dk*d**k* 比较大，softmax 放缩时就非常容易受影响，使不同值之间的差异较大，从而影响梯度的稳定性。因此，我们要将 Q 和 K 乘积的结果做一个放缩：
...
### [2.1.3 注意力机制的实现](https://datawhalechina.github.io/happy-llm/#/./chapter2/第二章 Transformer架构?id=_213-注意力机制的实现)
...
### [2.1.4 自注意力](https://datawhalechina.github.io/happy-llm/#/./chapter2/第二章 Transformer架构?id=_214-自注意力)
...
但在 Transformer 的 Encoder 结构中，使用的是 注意力机制的变种 —— 自注意力（self-attention，自注意力）机制。所谓自注意力，即是计算本身序列中每个元素对其他元素的注意力分布，即在计算过程中，Q、K、V 都由同一个输入通过不同的参数矩阵计算得到。在 Encoder 中，Q、K、V 分别是输入对参数矩阵 Wq、Wk、Wv*W**q*、*W**k*、*W**v* 做积得到，从而拟合输入语句中每一个 token 对其他所有 token 的关系。
...
attention(x, x, x)Copy to clipboardErrorCopied
```

### [2.1.5 掩码自注意力](https://datawhalechina.github.io/happy-llm/#/./chapter2/第二章 Transformer架构?id=_215-掩码自注意力)
...
Step 4：输入 【BOS】I like you，输出 【EOS】Copy to clipboardErrorCopied
```
...
<BOS>    I     like    you  【MASK】
<BOS>    I     like    you   </EOS>Copy to clipboardErrorCopied
```
...
mask = torch.triu(mask, diagonal=1)Copy to clipboardErrorCopied
```
...
### [2.1.6 多头注意力](https://datawhalechina.github.io/happy-llm/#/./chapter2/第二章 Transformer架构?id=_216-多头注意力)
...
![图片描述](https://raw.githubusercontent.com/datawhalechina/happy-llm/main/docs/images/2-figures/1-3.jpeg)
...
## [2.2 Encoder-Decoder](https://datawhalechina.github.io/happy-llm/#/./chapter2/第二章 Transformer架构?id=_22-encoder-decoder)
...
### [2.2.1 Seq2Seq 模型](https://datawhalechina.github.io/happy-llm/#/./chapter2/第二章 Transformer架构?id=_221-seq2seq-模型)

Seq2Seq，即序列到序列，是一种经典 NLP 任务。具体而言，是指模型输入的是一个自然语言序列 input=(x1,x2,x3...xn)*in**p**u**t*=(*x*1,*x*2,*x*3...*x**n*) ，输出的是一个可能不等长的自然语言序列 output=(y1,y2,y3...ym)*o**u**tp**u**t*=(*y*1,*y*2,*y*3...*y**m*) 。事实上，Seq2Seq 是 NLP 最经典的任务，几乎所有的 NLP 任务都可以视为 Seq2Seq 任务。例如文本分类任务，可以视为输出长度为 1 的目标序列（如在上式中 m*m* = 1）；词性标注任务，可以视为输出与输入序列等长的目标序列（如在上式中 m*m* = n*n* ）。
...
![图片描述](https://raw.githubusercontent.com/datawhalechina/happy-llm/main/docs/images/2-figures/2-0.jpg)
...
### [2.2.2 前馈神经网络](https://datawhalechina.github.io/happy-llm/#/./chapter2/第二章 Transformer架构?id=_222-前馈神经网络)
...
### [2.2.3 层归一化](https://datawhalechina.github.io/happy-llm/#/./chapter2/第二章 Transformer架构?id=_223-层归一化)
...
μj=1m∑i=1mZji*μ**j*=*m*1*i*=1∑*m**Z**j**i*

其中，Zji*Z**j**i* 是样本 i 在第 j 个维度上的值，m 就是 mini-batch 的大小。

再计算样本的方差：

σ2=1m∑i=1m(Zji−μj)2*σ*2=*m*1*i*=1∑*m*(*Z**j**i*−*μ**j*)2

最后，对每个样本的值减去均值再除以标准差来将这一个 mini-batch 的样本的分布转化为标准正态分布：

Zj~=Zj−μjσ2+ϵ*Z**j*=*σ*2+*ϵ**Z**j*−*μ**j*

此处加上 ϵ*ϵ* 这一极小量是为了避免分母为0。
...
class LayerNorm(nn.Module):
    ...
    # 注意这里也在最后一个维度发生了广播
    return self.a_2 * (x - mean) / (std + self.eps) + self.b_2Copy to clipboardErrorCopied
```

注意，在我们上文实现的 Layer Norm 层中，有两个线性矩阵进行映射。

### [2.2.4 残差连接](https://datawhalechina.github.io/happy-llm/#/./chapter2/第二章 Transformer架构?id=_224-残差连接)
...
x=x+MultiHeadSelfAttention(LayerNorm(x))*x*=*x*+*M**u**lt**i**He**a**d**S**e**l**f**A**tt**e**n**t**i**o**n*(*L**a**yer**N**or**m*(*x*))

output=x+FNN(LayerNorm(x))*o**u**tp**u**t*=*x*+*FNN*(*L**a**yer**N**or**m*(*x*))
...
### [2.2.5 Encoder](https://datawhalechina.github.io/happy-llm/#/./chapter2/第二章 Transformer架构?id=_225-encoder)
...
class EncoderLayer(nn.Module):
  '''Encoder层'''
    ...
    def forward(self, x):
        ...
        return outCopy to clipboardErrorCopied
```
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        for layer in self.layers:
            x = layer(x)
        return self.norm(x)Copy to clipboardErrorCopied
```

通过 Encoder 的输出，就是输入编码之后的结果。

### [2.2.6 Decoder](https://datawhalechina.github.io/happy-llm/#/./chapter2/第二章 Transformer架构?id=_226-decoder)
...
## [2.3 搭建一个 Transformer](https://datawhalechina.github.io/happy-llm/#/./chapter2/第二章 Transformer架构?id=_23-搭建一个-transformer)

在前两章，我们分别深入剖析了 Attention 机制和 Transformer 的核心——Encoder、Decoder 结构，接下来，我们就可以基于上一章实现的组件，搭建起一个完整的 Transformer 模型。

### [2.3.1 Embedding 层](https://datawhalechina.github.io/happy-llm/#/./chapter2/第二章 Transformer架构?id=_231-embedding-层)
...
```
[[[0],[1],[2]]]Copy to clipboardErrorCopied
```
...
```python
self.tok_embeddings = nn.Embedding(args.vocab_size, args.dim)Copy to clipboardErrorCopied
```

### [2.3.2 位置编码](https://datawhalechina.github.io/happy-llm/#/./chapter2/第二章 Transformer架构?id=_232-位置编码)
...
我们以一个简单的例子来说明位置编码的计算过程：假如我们输入的是一个长度为 4 的句子"I like to code"，我们可以得到下面的词向量矩阵 xx ，其中每一行代表的就是一个词向量， x0=[0.1,0.2,0.3,0.4]x0=[0.1,0.2,0.3,0.4] 对应的就是“I”的词向量，它的pos就是为0，以此类推，第二行代表的是“like”的词向量，它的pos就是1：

x=[0.10.20.30.40.20.30.40.50.30.40.50.60.40.50.60.7]x=0.10.20.30.40.20.30.40.50.30.40.50.60.40.50.60.7
...
[[ 0.          1.          0.          1.        ]
 ...
 [ 0.14112001 -0.9899925   0.29552021  0.95533649]]Copy to clipboardErrorCopied
```
...
f(⋯ ,xm,⋯ ,xn,⋯ )=f(⋯ ,xn,⋯ ,xm,⋯ )*f*(⋯,**x***m*,⋯,**x***n*,⋯)=*f*(⋯,**x***n*,⋯,**x***m*,⋯)

很明显，这样的函数是不具有不对称性的，也就是无法表征相对位置信息。我们想要得到这样一种编码方式：

f~(⋯ ,xm,⋯ ,xn,⋯ )=f(⋯ ,xm+pm,⋯ ,xn+pn,⋯ )*f*~(⋯,**x***m*,⋯,**x***n*,⋯)=*f*(⋯,**x***m*+**p***m*,⋯,**x***n*+**p***n*,⋯)

这里加上的 pm*p**m*， pn*p**n* 就是位置编码。接下来我们将 f(...,xm+pm,...,xn+pn)*f*(...,*x**m*+*p**m*,...,*x**n*+*p**n*) 在 m,n 两个位置上做泰勒展开：

f~≈f+pm⊤∂f∂xm+pn⊤∂f∂xn+12pm⊤∂2f∂xm2pm+12pn⊤∂2f∂xn2pn+pm⊤∂2f∂xm∂xnpn⏟pm⊤Hpn*f*~≈*f*+**p***m*⊤∂**x***m*∂*f*+**p***n*⊤∂**x***n*∂*f*+21**p***m*⊤∂**x***m*2∂2*f***p***m*+21**p***n*⊤∂**x***n*2∂2*f***p***n*+**p***m*⊤H**p***n***p***m*⊤∂**x***m*∂**x***n*∂2*f***p***n*

可以看到第1项与位置无关，2～5项仅依赖单一位置，第6项（f 分别对 m、n 求偏导）与两个位置有关，所以我们希望第六项（ pmTHpn*p**m**T**H**p**n* ）表达相对位置信息，即求一个函数 g 使得:

pmTHpn=g(m−n)*p**m**T**H**p**n*=*g*(*m*−*n*)

我们假设 H*H* 是一个单位矩阵，则：

pmTHpn=pmTpn=⟨pm,pn⟩=g(m−n)*p**m**T**H**p**n*=*p**m**T**p**n*=⟨**p***m*,**p***n*⟩=*g*(*m*−*n*)

通过将向量 [x,y] 视为复数 x+yi，基于复数的运算法则构建方程:

⟨pm,pn⟩=Re[pmpn∗]⟨**p***m*,**p***n*⟩=Re[**p***m***p***n*∗]

再假设存在复数 qm−n*q**m*−*n* 使得：

pmpn∗=qm−n**p***m***p***n*∗=**q***m*−*n*

使用复数的指数形式求解这个方程，得到二维情形下位置编码的解：

pm=eimθ⇔pm=(cos⁡mθsin⁡mθ)**p***m*=*e*i*m**θ*⇔**p***m*=(cos*m**θ*sin*m**θ*)

由于内积满足线性叠加性，所以更高维的偶数维位置编码，我们可以表示为多个二维位置编码的组合：

pm=(eimθ0eimθ1⋮eimθd/2−1)⇔pm=(cos⁡mθ0sin⁡mθ0cos⁡mθ1sin⁡mθ1⋮cos⁡mθd/2−1sin⁡mθd/2−1)**p***m*=*e*i*m**θ*0*e*i*m**θ*1⋮*e*i*m**θ**d*/2−1⇔**p***m*=cos*m**θ*0sin*m**θ*0cos*m**θ*1sin*m**θ*1⋮cos*m**θ**d*/2−1sin*m**θ**d*/2−1

再取 θi=10000−2i/d*θ**i*=10000−2*i*/*d*（该形式可以使得随着|m−n|的增大，⟨pm,pn⟩有着趋于零的趋势，这一点可以通过对位置编码做积分来证明，而 base 取为 10000 是实验结果），就得到了上文的编码方式。

当 H*H* 不是一个单位矩阵时，因为模型的 Embedding 层所形成的 d 维向量之间任意两个维度的相关性比较小，满足一定的解耦性，我们可以将其视作对角矩阵，那么使用上述编码：

pm⊤Hpn=∑i=1d/2H2i,2icos⁡mθicos⁡nθi+H2i+1,2i+1sin⁡mθisin⁡nθi**p***m*⊤H**p***n*=*i*=1∑*d*/2H2*i*,2*i*cos*m**θ**i*cos*n**θ**i*+H2*i*+1,2*i*+1sin*m**θ**i*sin*n**θ**i*

通过积化和差：

∑i=1d/212(H2i,2i+H2i+1,2i+1)cos⁡(m−n)θi+12(H2i,2i−H2i+1,2i+1)cos⁡(m+n)θi*i*=1∑*d*/221(H2*i*,2*i*+H2*i*+1,2*i*+1)cos(*m*−*n*)*θ**i*+21(H2*i*,2*i*−H2*i*+1,2*i*+1)cos(*m*+*n*)*θ**i*

说明该编码仍然可以表示相对位置。

上述编码结果，如图2.6所示：

![图片描述](https://raw.githubusercontent.com/datawhalechina/happy-llm/main/docs/images/2-figures/3-0.png)
...
### [2.3.3 一个完整的 Transformer](https://datawhalechina.github.io/happy-llm/#/./chapter2/第二章 Transformer架构?id=_233-一个完整的-transformer)
...
class Transformer(nn.Module):
   '''整体模型'''
    ...
    def forward(self, idx, targets=None):
        ...
        return logits, lossCopy to clipboardErrorCopied
```
