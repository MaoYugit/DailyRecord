...
**不要直接在 setup 中解构 props。**

- *原因*：解构会丢失响应性。
- *对策*：如果需要解构，应使用 toRefs。这是一个常见的 Bug 来源，值得提醒。
    ...
    <template>
      <div>
        ...
        <ChildComponent v-bind="$attrs" />
          <script setup>
    // 必须加这一行！否则即使没有接收属性也会因为默认继承把属性都应用到该组件的跟元素上
    defineOptions({
      inheritAttrs: false
    })
    </script>
...
| 父传子                        | props              |
| 子传父                        | emit               |
| 跨级传递（祖孙）              | provide / inject   |
| 全局状态（用户登录、购物车）  | Pinia              |
| 无关系组件（跨页面通知）      | mitt               |
| 封装原生组件（Button, Input） | $attrs             |
| 外部控制子组件行为            | ref + defineExpose |
| 高度定制 UI 结构              | slots              |
