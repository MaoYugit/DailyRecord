...
**1. 安全性：你可以在入口处设置“安检” (Data Validation)**

这是最直观、最重要的一个优点。通过 setter 方法，你可以加入逻辑判断，过滤掉不合法的数据。

**如果没有封装：**
别人可以随意设置任何值，甚至是毫无意义或危险的值。



**2.灵活性：你可以决定属性是“只读”还是“只写”**

有时候，某些属性一旦被创建就不应该被修改（例如学生的ID号），或者只能被内部修改。

- **只读（Read-Only）属性**：只提供 getter 方法，不提供 setter 方法。

**3.维护性：隐藏内部实现，方便未来修改**

假设你有一个 `Person` 类，它有 `getAge()` 方法。 

* **版本1**：内部用一个 `int age` 变量存储。

* **版本2**：后来需求变了，你不想存年龄，而是想存出生日期 `Date birthday`，然后通过计算得出年龄。

   如果你用的是公共属性 `public int age;`，那么所有使用到 `person.age` 的代码在版本2中都必须重写，因为 `age` 这个属性已经不存在了，这是一个巨大的灾难。 但如果你用的是封装，只改一下getAge的逻辑就好了
