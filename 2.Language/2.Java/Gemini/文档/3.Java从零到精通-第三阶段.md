...
* **【常用方法实践】**
  ...
  public class StringApiPractice {
      public static void main(String[] args) {
          ...
          // 2. 获取指定索引的字符
          System.out.println("第4个字符: " + text.charAt(3)); // 'e' (索引从0开始)
...
* **【`StringBuilder`实践】**
  ...
  **法则：** 在循环外或单次操作用`String`，在循环内或多次拼接用`StringBuilder`。
  ...
  `StringBuffer` 是 Java 中一个用来处理字符串的类，它位于 `java.lang` 包中，因此无需额外导入。
  
  它的核心特点是：**它是一个可变的、线程安全的字符序列**。
  
  *   **可变 (Mutable)**：与 `String` 对象一旦创建就无法修改相反，`StringBuffer` 对象的内容可以随时被修改（增加、删除、替换），而不会创建新的对象。这使得它在需要频繁拼接或修改字符串的场景下非常高效。
  *   **线程安全 (Thread-Safe)**：`StringBuffer` 的所有公共方法（如 `append`, `insert` 等）都是 `synchronized`（同步的）。这意味着在多线程环境中，多个线程可以同时访问同一个 `StringBuffer` 对象而不会导致数据错乱。这是它与 `StringBuilder` 最主要的区别。
  
  #### 2. 为什么需要 StringBuffer？（与 String 的对比）
  
  我们知道 `String` 是不可变的。当你执行下面的代码时：
  ...
  这并不是在修改原始的 "hello" 字符串。实际上，Java 虚拟机 (JVM) 创建了一个全新的字符串 "hello world"，然后让 `str` 变量指向这个新对象。如果在循环中大量进行这样的操作，会产生非常多的垃圾对象，影响性能。
  
  `StringBuffer` 就是为了解决这个问题而生的。它内部维护一个字符数组，当你追加或修改内容时，它会直接在这个数组上操作，只有当数组空间不足时才会进行扩容，这比每次都创建新对象要高效得多。
  
  #### 3. StringBuffer 与 StringBuilder 的对比
  
  | 特性         | String                 | StringBuffer                     | StringBuilder                  |
  | :----------- | :--------------------- | :------------------------------- | :----------------------------- |
  | **可变性**   | **不可变**             | **可变**                         | **可变**                       |
  | **线程安全** | 线程安全               | **线程安全** (方法同步)          | **非线程安全**                 |
  | **性能**     | 频繁修改时性能最低     | 较低 (因同步有开销)              | **最高**                       |
  | **适用场景** | 字符串不经常改变的场景 | **多线程**环境下共享、操作字符串 | **单线程**环境下大量操作字符串 |
  
  **简单记：**
  *   `StringBuilder`：单线程下的“加强版”`StringBuffer`，速度最快。
  *   `StringBuffer`：多线程下的安全选择，速度稍慢。
  
  #### 4. 常用方法
  
  `StringBuffer` 的方法和 `StringBuilder` 几乎完全一样：
  
  *   `append( ... )`: 在末尾追加内容。可以追加字符串、基本数据类型等。这是最常用的方法。
  *   `insert(int offset, ... )`: 在指定位置插入内容。
  *   `delete(int start, int end)`: 删除从 `start` 到 `end-1` 位置的字符。
  *   `replace(int start, int end, String str)`: 用指定的字符串替换从 `start` 到 `end-1` 的内容。
  *   `reverse()`: 将缓冲区中的内容反转。
  *   `length()`: 返回当前内容的长度。
  *   `capacity()`: 返回当前缓冲区的容量（内部数组的大小）。
  *   `toString()`: **非常重要！** 将 `StringBuffer` 的内容转换为一个不可变的 `String` 对象。
  
  ---
  
  ### 使用小案例：构建一个多线程安全的日志记录器
  
  假设我们有一个简单的日志记录器，多个任务（线程）会同时向这个记录器里写入日志。如果使用 `StringBuilder`，可能会因为并发写入导致日志内容错乱。这时，`StringBuffer` 就是理想的选择。
  
  **场景**：模拟两个支付渠道（支付宝和微信）并发完成支付，并向同一个日志缓冲区写入成功记录。
  
  ```java
  package learn.maoyu.api.buffers;
  
  public class SafeLogger {
  
      // 使用 StringBuffer 作为共享的日志缓冲区，保证线程安全
      private static final StringBuffer logBuffer = new StringBuffer();
  
      public static void main(String[] args) throws InterruptedException {
          logBuffer.append("--- 系统启动，开始记录日志 ---\n");
  
          // 线程1：模拟支付宝支付回调
          Thread alipayThread = new Thread(() -> {
              log("Alipay", "AP20251118001", 150.75);
          });
  
          // 线程2：模拟微信支付回调
          Thread wechatPayThread = new Thread(() -> {
              log("WeChatPay", "WP20251118002", 88.00);
          });
  
          // 启动两个线程，它们会并发地调用 log 方法
          alipayThread.start();
          wechatPayThread.start();
  
          // 等待两个线程都执行完毕
          alipayThread.join();
          wechatPayThread.join();
  
          logBuffer.append("--- 所有任务完成，日志记录结束 ---\n");
  
          // 最后，将日志缓冲区的内容转换成 String 并打印出来
          System.out.println("最终的日志记录:");
          System.out.println(logBuffer.toString());
      }
  
      /**
       * 记录日志的方法
       * 因为 logBuffer.append 是同步的，所以这个方法是线程安全的
       * @param channel 支付渠道
       * @param orderId 订单号
       * @param amount 金额
       */
      public static void log(String channel, String orderId, double amount) {
  ...
  1.  **共享资源**：`logBuffer` 是一个 `static final` 的 `StringBuffer` 对象，被 `main` 线程和我们创建的两个子线程（`alipayThread`, `wechatPayThread`）所共享。
  2.  **并发操作**：两个支付线程并发执行，它们会几乎同时调用 `log` 方法，对 `logBuffer` 进行 `append` 操作。
  3.  **线程安全**：因为 `StringBuffer` 的 `append` 方法是同步的，所以同一时刻只允许一个线程修改 `logBuffer`。这可以防止出现日志信息交叉或覆盖的混乱情况（比如 `[SUCCESS] Channel: Alipay, OrderId: WP2025...` 这种错乱）。
  4.  **最终输出**：当所有线程执行完毕后，我们调用 `.toString()` 方法，从 `StringBuffer` 中获取一个完整的、最终的日志字符串并打印。无论两个线程谁先谁后完成，最终的日志记录都是完整且清晰的。
  
  这个案例完美地展示了 `StringBuffer` 在多线程共享数据场景下的核心价值。如果在单线程环境下，比如在一个循环里构建一个复杂的字符串，那么使用性能更高的 `StringBuilder` 会是更好的选择。
...
**编程学习不是冲刺，而是马拉松。感到困惑时，停下来，多看几遍，多敲几遍代码，理解自然就会水到渠成。
